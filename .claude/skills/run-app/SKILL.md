---
name: run-app
description: Use this skill when the user says "run the app", "lance l'application", "démarre l'app", or asks to start/preview Competitions WebApp locally. Starts the Express server (server/index.js) on localhost:8080, which serves both the REST API and the built Angular SPA (client-angular/), and opens the app.
---

# Run Competitions WebApp locally (Angular frontend, experimental branch)

This branch (`AngularFrontEnd`) replaces the vanilla-JS/`.htm` frontend with an Angular SPA in
`client-angular/` (Angular CLI, application builder). A single Express process
(`server/index.js`) serves both the REST API (`/api/*`) and the built SPA, on **port 8080**
(fixed by convention for this app — do not change it).
...
client-side routes handled by Angular Router (Express falls back to the SPA shell for any
...
1. Confirm the current directory looks like the right project (sanity check, don't hardcode
   a path):
   ```bash
   test -d client-angular -a -d server -a -d data && echo OK
   ```
...
4. Build the Angular app if it hasn't been built yet, or if source files changed since the
   last build (this branch serves the production build, not a dev server — rebuild after
   editing `client-angular/src/**`):
   ```bash
   (cd client-angular && npm install && npx ng build)
   ```
